...
def get_ai_response(prompt):
    sys_prompt = f"You are a Pakistani Agri-Expert. Reply ONLY in {lang} script. NO Chinese/Hindi. Use 'Assalam-o-Alaikum'."
    messages = [{"role": "system", "content": sys_prompt}]
    messages.extend(st.session_state.messages[-3:]) # Context
...
if menu == "💬 چیٹ":
    ...
    st.write("---")
    # Sab se niche Input Box (Drag/Voice option yahan aayega)
    st.subheader("🎤 اپنا نیا سوال پوچھیں")
    c1, c2 = st.columns([1, 4])
    with c1:
        audio_data = mic_recorder(start_prompt="🎤", stop_prompt="⏹️", key='chat_mic')
    with c2:
        u_text = st.text_input("Yahan likhein...", label_visibility="collapsed", key="chat_in")
    ...
    if audio_data and audio_data.get('id') != st.session_state.processed_id:
        ...
        trans = client.audio.transcriptions.create(file=("audio.wav", audio_data['bytes']), model="whisper-large-v3", language="ur")
        final_q = trans.text
    elif send and u_text:
        final_q = u_text

    if final_q:
        ans = get_ai_response(final_q)
    ...
    if file:
        ...
        if st.button("ڈاکٹر سے مشورہ لیں"):
            ...
            play_audio(ans)
            st.info("مزید سوالات کے لیے چیٹ مینو میں جائیں۔")

elif menu == "🧪 کھاد ایڈوائزر":
    st.subheader("کھاد کا مشورہ")
    q = st.text_input("Zameen ki halat likhein:")
    if st.button("Mashwara"):
    ...
    if st.button("Rate"):
        ans = get_ai_response(f"Mandi rates for {crop}")
        st.markdown(f"<div class='urdu-font'>{ans}</div>", unsafe_allow_html=True)
        play_audio(ans)
